...
$$c_{ij} = \sum_{k=-\infty}^\infty (\rho_{k+i}\rho_{k+j} + \rho_{k-i}\rho_{k+j} - 2\rho_i\rho_k\rho_{k+j} - 2\rho_j\rho_k\rho_{k+i} + 2\rho_i\rho_j\rho_k^2).$$
    ...
    the asymptotic marginal distribution of $r_m$:
    $$\sqrt{n} (r_m - \rho_m)
      \overset{\mathcal{D}}\longrightarrow \mathrm{N}(0, c),$$
    where
...
in question using Gaussian noise:
    ...
    the model in question:
        ...
        var_r_4 <- (1 + 2 * sum(rho_values^2)) / 100
        var_r_4
        ```
    ...
    histogram of the resulting $r_4$ values:
        ...
        for (i in 1:length(r_4_values)) {
          p <- arima.sim(n = 100, model = list(ma = -c(0.7, 0.5, 0.6)))
    ...
    for $\var(r_4)$. The following computes this estimator on the originally
    generated data:
    
        ```{r}
        acf_values <- acf(process, plot = FALSE)$acf[1:3]
        var_r_4_est <- (1 + 2 * sum(acf_values^2)) / 100
        var_r_4_est
        ```
